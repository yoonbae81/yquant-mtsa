...
class ConfigManager(private val context: Context) {
    ...
    fun loadLoginCertConfig(): LoginCertConfig? {
        ...
        return resolveLoginCertConfig(parsedConfig = null, savedCertPassword = savedCertPassword)
    }

    fun loadCredentialSettings(): CredentialSettings {
        val store = SecureCredentialStore(context)
        return CredentialSettings(
            certPassword = store.getString(CREDENTIAL_CERT_PASSWORD_KEY),
        )
    }
    ...
    fun saveCredentialSettings(settings: CredentialSettings): Boolean {
        return try {
            val store = SecureCredentialStore(context)
            store.putString(CREDENTIAL_CERT_PASSWORD_KEY, settings.certPassword)
